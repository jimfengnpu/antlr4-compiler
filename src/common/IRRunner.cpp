...
void IRRunner::operateScalObj(IRType type, pIRObj target, pIRObj opt1,
                              pIRObj opt2) {
...
void IRRunner::runSysY(const SysYIR &instr) {
    ...
    if (instr.target != nullptr) cout << " " << instr.target.get()->name;
    if (instr.opt1 != nullptr) cout << ", " << instr.opt1.get()->name;
    if (instr.opt2 != nullptr) cout << ", " << instr.opt2.get()->name << endl;
#endif
    pIRScalValObj targ = nullptr;
    ...
    switch (instr.type) {
        ...
        case IRType::CALL:
            ...
            if (func->symbolTable == nullptr) {
                ...
                for (auto &obj : func->params) {
                    ...
                    if (auto arg = toScal(*rParam)) {
                        ...
                        auto arrArg =
                            dynamic_pointer_cast<IRArrValObj>(*rParam);
            ...
            paramsBuf.clear();
            if (func->returnType == IR_INT) storeValue(targ, rt);
            break;
        case IRType::ARR:
